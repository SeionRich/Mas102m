...
```{r, fig.align = "center", fig.cap = "Scatter plot of discarded fish species weights across bottom trawl fishing tows. A total of 48 bottom trawl fishing tows were made. Most of the weight observations are below 125 kgs" }
...
ggplot(data = Top.6.fish.data,
  ...
  geom_text(aes(label = ifelse(TotalWeightKG == max(TotalWeightKG),
    paste0("Max = ",round(TotalWeightKG, 3)), "")), 
    hjust = -0.5,
    size = 3,
    color = "red") + 
  geom_text(aes(label = ifelse(TotalWeightKG == min(TotalWeightKG),
    paste0("Min = ", round(TotalWeightKG, 3)), "")),
    vjust = 1.5,
    size = 3,
    color = "red") + 
...
```{r, fig.align = "center", fig.cap = "Scatter plot of discarded fish species weights across bottom trawl fishing tows. The oservations above 125 kgs removed - possible outliers. Points on the plot represent discarded fish species"}
...
Without.Outliers <- Top.6.fish.data %>% 
  filter(TotalWeightKG < 125)
...
ggplot(data = Top.6.fish.data) +
  ...
  geom_text(data = Outliers,
    aes(x = DragID2,
      y = TotalWeightKG,
      label = round(TotalWeightKG,3),
      color = LatinNames),
    hjust = -0.5,
    size = 3) +
  geom_text(data = Without.Outliers,
    aes(x = DragID2,
      y = TotalWeightKG,
      label = ifelse(TotalWeightKG == max(TotalWeightKG),
        paste0("New max = ",round(TotalWeightKG, 3)), "")), 
    hjust = -0.5,
    size = 3,
    color = "red") + 
  geom_text(aes(x = DragID2,
    y = TotalWeightKG,
    label = ifelse(TotalWeightKG == min(TotalWeightKG),
      paste0("Min = ", round(TotalWeightKG, 3)), "")),
    vjust = 1.5,
    size = 3,
    color = "red") + 
...
From Figure 3.2 we see that there were three abnormally high data points. These were for discarded fish species *Trichiurus lepturus*(219.586 kgs), *Stellifer rastrifer*(163.973 kgs) and *Stellifer microps*(132.773 kgs). As mentioned before these points are strangely high and therefore are perceived to be incorrectly entered and thus will be omitted from the analysis. From the Figure we also see that the minimum weight was 0.018 kgs and the *new* maximum will be 107.599 kgs. 
...
**Creating a table with the top 6 fish species weights across time of day**
...
**Creating a table of the top 6 fish species weights across fishing depths**
...
To answer this research question we will now look at the discarded fish species - weight data for all fish species. This will help us to get an idea of how weights are distributed for the entire population and not just a subsample of the data which we used to answer *Research Question 3*.
...
# Manually calculating data minimum to deal with problem of 
# multiple minimum values
min.wt <- min(round(Discard.data$TotalWeightKG, 3))
...
ggplot(data = Discard.data,
  ...
  geom_text(aes(label = ifelse(TotalWeightKG == max(TotalWeightKG),
    ...
    size = 3,
    color = "red") + 
  geom_text(aes(x = 4, y = -7,
    label = paste0("Min = ", min.wt)),
    size = 3,
    color = "red") +
  guides(colour = guide_legend(override.aes = aes(label = "")))
...
ggplot(data = Discard.data) +
  ...
  geom_text(data = Outliers.2,
    ...
    hjust = -0.5,
    size = 3) +
  geom_text(data = Without.Outliers.2,
    aes(x = DragID2,
      y = TotalWeightKG,
      label = ifelse(TotalWeightKG == max(TotalWeightKG),
        paste0("New max = ", round(TotalWeightKG, 3)), "")), 
  ...
  geom_text(aes(x = 4, y = -7,
    ...
    size = 3,
    color = "red") +
  guides(colour = guide_legend(override.aes = aes(label = ""), nrow = 2))

### CODE COMMENTS ###
#[1]. Please see the code comments made at Figure 3.2.  
```

From Figure 3.6 we see that the they were five abnormally high data points. These were for discarded fish species *Trichiurus lepturus*(219.586 kgs), *Symphurus plagusia*(178.964 kgs), *Stellifer rastrifer*(163.973 kgs), *Dasyatis geijskesi*(161.077 kgs) and *Stellifer microps*(132.773). As mentioned before these points are strangely high and therefore are perceived to be incorrectly entered and thus will be omitted from the analysis. 
